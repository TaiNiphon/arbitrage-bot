...
class TitanV18_The_Precision:
    def __init__(self):
        ...
        self._init_db() 
        self._load_state() 
        self.notify("🏛️ <b>TITAN V.18.28: AUTO-CLEANUP ENABLED</b>\n<i>Status: Monitoring Wallet vs Database Sync</i>")
        ...
        while True:
            try:
                ...
                if res and 'result' in res:
                    ...
                    if coin < 0.0001: # ถ้าเหรียญหมดแต่ใน Database ยังคิดว่าถืออยู่
                        with psycopg2.connect(self.db_url) as conn:
                            with conn.cursor() as cur:
                                cur.execute("DELETE FROM bot_state_v18 WHERE status='MATCHED'")
                                conn.commit()
                        self._load_state() # รีโหลดสถานะใหม่ทันที
