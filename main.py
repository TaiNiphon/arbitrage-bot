...
class TitanV18_UltimateLuxury:
    def __init__(self):
        ...
        # 2. Trading Strategy Constants
        self.initial_equity = 10000.28 
        self.buy_rsi_threshold = 28.0      
        self.target_profit = 3.0 # TP 3%
        self.slots = {1: {"status": "FREE"}, 2: {"status": "FREE"}}

        self._load_state()
        self.notify("🏛️ <b>TITAN V.18.99: FINAL LUXURY ONLINE</b>\n<i>Status: Database Synced & Engine Ready</i>")

    def get_thai_now(self):
        return datetime.now(timezone(timedelta(hours=7)))

    def _load_state(self):
        """ดึงข้อมูลจาก Postgres ให้ตรงกับภาพ 7980.jpg 100%"""
        try:
    ...
    def send_luxury_dashboard(self, dx, db_btc, thb, coin):
        """รายงานหน้าตาเหมือนภาพ 7978.jpg ทุกประการ (ห้ามตัดทอน)"""
    ...
    def execute_trade(self, side, slot_id, price, amt_val, buy_p=0):
        """ระบบเทรดที่บันทึกประวัติลงภาพ 7982.jpg ครบถ้วน"""
        ...
        if res and res.get('error') == 0:
            time.sleep(2) # รอ Ledger ฝั่ง Bitkub
            with psycopg2.connect(self.db_url) as conn:
                with conn.cursor() as cur:
                    if side == 'buy':
                        # บันทึกตาม Column ในภาพ 7980.jpg
                        cur.execute("""INSERT INTO bot_state_v18 
                            (slot_id, price, units, sl, max_p, status, open_ts, order_id) 
                            VALUES (%s,%s,%s,%s,%s,%s,%s,%s) 
                            ON CONFLICT (slot_id) DO UPDATE SET 
                            price=EXCLUDED.price, units=EXCLUDED.units, sl=EXCLUDED.sl, status='MATCHED'""", 
                            (slot_id, price, (amt_val/price), (price*0.95), price, 'MATCHED', int(time.time()*1000), f"TITAN_{int(time.time())}"))
                    else:
                        # บันทึกตาม Column ในภาพ 7982.jpg
                        pnl = (price * amt_val * 0.9975) - (buy_p * amt_val * 1.0025)
                        status_str = "PROFIT" if pnl > 0 else "LOSS"
                        cur.execute("""INSERT INTO trade_history (slot_id, side, price, units, net_pnl_thb, ts, status) 
                                       VALUES (%s,%s,%s,%s,%s,%s,%s)""", 
                                    (slot_id, "SELL", price, amt_val, pnl, self.get_thai_now(), status_str))
                        cur.execute("DELETE FROM bot_state_v18 WHERE slot_id=%s", (slot_id,))
                    conn.commit()
            
            self._load_state() # Sync ทันทีหลังเทรด
            self.notify(f"✅ <b>{side.upper()} COMPLETED</b>\nPrice: {price} | Slot: {slot_id}")
            return True
        return False
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                if dx and db_btc:
                    # รายงานรายชั่วโมง (Luxury Dash)
                    if self.get_thai_now().hour != last_h:
                        self._load_state()
                        self.send_luxury_dashboard(dx, db_btc, thb, coin)
                        last_h = self.get_thai_now().hour
                    
                    # 🔴 LOGIC ขาย (TP/SL) - ตรวจสอบจาก Database ตลอดเวลา
                    for i, s in self.slots.items():
                        if s.get('status') == 'MATCHED':
                            profit = ((dx['p'] * 0.9975) / (s['price'] * 1.0025) - 1) * 100
                            # SL ทำงานเมื่อราคา <= sl ใน DB | TP ทำงานเมื่อ profit >= target
                            if profit >= self.target_profit or dx['p'] <= s['sl']:
                                self.execute_trade('sell', i, dx['p'], s['units'], buy_p=s['price'])
                    
                    # 🔵 LOGIC ซื้อ (RSI + Money Management)
                    active_count = sum(1 for s in self.slots.values() if s.get('status') == 'MATCHED')
                    if active_count < 2 and dx['r14'] <= self.buy_rsi_threshold:
                        if thb >= 10:
                            target_slot = 1 if self.slots[1].get('status') != 'MATCHED' else 2
                            # แก้ปัญหาไม้ 2: แบ่งเงิน 48% สำหรับไม้แรก ถ้ามีไม้แรกแล้วใช้ 96% ของที่เหลือ
                            buy_amt = int(thb * 0.48) if active_count == 0 else int(thb * 0.96)
                            self.execute_trade('buy', target_slot, dx['p'], buy_amt)
            except Exception as e:
