...
class TitanOmniV13_3:
    def __init__(self):
        ...
        # --- New Safety Features (V.13.3) ---
        self.daily_drawdown_limit = 5.0 # หยุดเทรดเมื่อขาดทุนเกิน 5% ของ Initial Equity ในวันนั้น
        self.daily_pnl = 0.0 # สะสมกำไร/ขาดทุนรายวัน
        self.last_pnl_reset = datetime.now(timezone(timedelta(hours=7))).date()
        self.is_bot_active = True # สถานะเปิด/ปิดบอทอัตโนมัติ

        self._init_db()
        self._sync_with_wallet() 
        self.notify("<b>🛡️ TITAN OMNI V.13.3 PLUS | ACTIVE</b>\n<i>Status: Daily Drawdown & Heartbeat Enabled</i>")
    ...
    def _init_db(self):
        try:
            ...
            # เพิ่มตารางเก็บประวัติการเทรดเพื่อคำนวณ Daily Drawdown
            ...
            self.notify(f"⚠️ <b>DB Init Error:</b> {e}")
    ...
    def _sync_with_wallet(self):
        try:
            ...
            if coin > 0.1: 
                ...
                self.avg_price = row[1] if row and row[1] > 0 else 46.18 # ไม้เก่า 46.18
            ...
            self.notify(f"⚠️ <b>Sync Error:</b> {e}")

    def _save_trade_history(self, side, price, units, pnl_thb):
        try:
            conn = psycopg2.connect(self.db_url); cur = conn.cursor()
            cur.execute("INSERT INTO trade_history (side, price, units, pnl_thb) VALUES (%s, %s, %s, %s)", 
                        (side, price, units, pnl_thb))
            conn.commit(); cur.close(); conn.close()
        except: pass
    ...
    def _update_daily_pnl(self):
        try:
            ...
            if today > self.last_pnl_reset:
                ...
                self.is_bot_active = True # รีเซ็ตสถานะบอทเมื่อขึ้นวันใหม่

            conn = psycopg2.connect(self.db_url); cur = conn.cursor()
            cur.execute("SELECT SUM(pnl_thb) FROM trade_history WHERE traded_at::date = %s", (today,))
            result = cur.fetchone()
            self.daily_pnl = float(result[0]) if result[0] else 0.0
    ...
    def run(self):
        ...
        while True:
            try:
                self._update_daily_pnl() # อัปเดต Daily PnL ทุก Loop

                d15 = self.get_indicators("15")
                d60 = self.get_indicators("60")
                if not d15 or not d60: 
                ...
                if dd_pct <= -self.daily_drawdown_limit and self.is_bot_active:
                    self.is_bot_active = False # หยุดเทรนด์ฝั่งซื้อ
                    self.notify(f"🚨 <b>DAILY DRAWDOWN ALERT:</b> บอทหยุดเทรนด์ฝั่งซื้ออัตโนมัติเนื่องจากขาดทุนรวมวันนี้เกิน {self.daily_drawdown_limit}% ({self.daily_pnl:,.2f} THB)")

                # --- BUY LOGIC (ต้อง Active และผ่าน Filter) ---
                ...
                # --- SELL LOGIC (Trailing Stop & Breakeven) ---
                elif self.last_action == "buy" and coin > 0.1:
                    self.highest_price = max(self.highest_price, p)
                    trail_dist = d15['atr'] * 2.5
                    if p - trail_dist > self.dynamic_sl: 
                        self.dynamic_sl = p - trail_dist
                        self._save_state()
                    
                    if pnl >= 1.0: # Lock ทุนเมื่อกำไรเกิน 1%
                    ...
                    if pnl >= 10.0 or p <= self.dynamic_sl:
                        if self.place_order("sell", coin):
                            ...
                            self._save_trade_history("sell", p, coin, pnl_thb) # บันทึกประวัติการขาย
                            self.notify(f"💰 <b>EXIT SELL: {p:,.2f}</b>\nProfit: {pnl:+.2f}% ({pnl_thb:,.2f} THB)")
                            self.last_action = "sell"; self.avg_price = 0; self.total_units = 0; self.dynamic_sl = 0
                            self._save_state()

                # --- REPORTING (เหมือนเดิมเป๊ะ) ---
                if time.time() - last_rep >= self.report_interval:
                    ...
                    msg = (f"🛡️ <b>TITAN V.13.3 | {self.symbol}</b>\n"
                           f"Status : {status}\n"
                ...
                # --- Heartbeat Monitor (V.13.3) ---
                if time.time() - last_heartbeat >= 21600: # ทุก 6 ชั่วโมง
                    dd_pct = (self.daily_pnl / self.initial_equity) * 100
                    status_txt = "ACTIVE ✅" if self.is_bot_active else "PAUSED 🛑 (Drawdown Limit)"
                    heartbeat_msg = (f"💓 <b>TITAN Heartbeat</b>\nStatus: {status_txt}\nDaily PnL: {self.daily_pnl:,.2f} THB ({dd_pct:+.2f}%)\nInitial Equity: {self.initial_equity:,.2f} THB")
                    self.notify(heartbeat_msg)
                    last_heartbeat = time.time()

            except Exception as e: 
                self.notify(f"🚨 <b>Loop Error:</b> {e}")
    ...
    def place_order(self, side, amt):
        try:
            ...
            self.notify(f"🚨 <b>Order Crash:</b> {e}")
            return False
