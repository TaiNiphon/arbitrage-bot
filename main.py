...
class TitanOmniV15_Pro:
    def __init__(self):
        ...
        # --- Strategy & Risk ---
        # ปรับปรุงการดึงค่า Equity ให้ยืดหยุ่นขึ้น
        ...
        # เริ่มต้นระบบฐานข้อมูลและซิงค์ข้อมูล
        self._init_db_v15()
        self._sync_slots_from_db()
        self.notify("<b>🔥 TITAN V.15.1 PRO | DEPLOYED</b>\n<i>System: Stability Base Active</i>")
    ...
    def get_balance(self):
        try:
            ...
            if res.get('error') == 0:
                ...
                return float(res['result'].get('THB', 0)), float(res['result'].get(coin_name, 0))
        except: pass
        return 0.0, 0.0

    def get_indicators(self):
        try:
            res = requests.get(f"https://api.bitkub.com/tradingview/history?symbol={self.symbol}&resolution=15&from={int(time.time())-86400}&to={int(time.time())}", timeout=15).json()
            if 'c' not in res or not res['c']:
                return None # ส่งกลับ None เพื่อให้ Loop หลักข้ามรอบนี้ไป
    ...
    def place_smart_order(self, side, amt, price):
        try:
            ...
            r = requests.post(f"https://api.bitkub.com{path}", headers={'X-BTK-APIKEY': self.api_key, 'X-BTK-TIMESTAMP': ts, 'X-BTK-SIGN': sig}, data=json.dumps(payload), timeout=15)
            return r.json().get('error') == 0
        except:
            return False

    def run(self):
        # บังคับส่ง Report แรกทันทีเพื่อให้ทราบว่าบอทออนไลน์
        last_rep = 0 
        print("TITAN V.15.1 PRO Running...")
        
        while True:
            try:
                d = self.get_indicators()
                if d is None:
                    time.sleep(10); continue # ข้ามรอบถ้า API คืนค่าผิดพลาด
                ...
                for s_id, s in self.slots.items():
                    if s["active"]:
                        ...
                        if p - (atr * 2.5) > s['sl']: s['sl'] = p - (atr * 2.5) # Trailing Stop
                        
                        if p <= s['sl'] or curr_pnl >= 10.0:
                            if self.place_smart_order("sell", s['units'], p):
                                self.notify(f"📤 <b>CLOSE SLOT {s_id} @ {p:,.2f}</b>\nPnL: {curr_pnl:+.2f}%")
                                self.slots[s_id] = {"active": False, "price": 0.0, "units": 0.0, "sl": 0.0, "max_pnl": 0.0}
                                self._save_state(s_id)
            ...
            time.sleep(15) # ปรับให้ช้าลงเล็กน้อยเพื่อความเสถียรของ API
    ...
    def _report(self, p, rsi, equity, growth, thb, coin):
        try:
            ...
            msg = (f"🛡️ <b>TITAN V.15.1 PRO | {self.symbol}</b>\nStatus : {status}\nDate : {now.strftime('%d/%m/%Y')}\nTime : {now.strftime('%H:%M:%S')}\n"
