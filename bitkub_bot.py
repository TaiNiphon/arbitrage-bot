...
# --- 2. ฟังก์ชันแก้ปัญหาเครื่องหมายอัญประกาศ (") อัตโนมัติ ---
def get_clean_env(key):
    val = os.getenv(key, "")
    # ล้างเครื่องหมาย " และ ' และช่องว่างออกให้หมด
...
send_line_msg("🤖 บอท Bitkub XRP เริ่มทำงานแล้ว!")
...
while True:
    try:
        ...
        if wallet and wallet.get('error') == 0:
            ...
            # แจ้งรายละเอียด Error ลงใน Log เพื่อใช้ตรวจสอบ
